...
Franklin_Transfers_To_Col <- as.Date(Franklin_LB$Transfers_To_Col)
...
```{r}
Hamilton_School_Data %>% group_by(AUDPTYID) %>%  count(NAME.y) 
...
Because we had to geolocate the parcels within the school districts, this wipes our previous data, so adding it back in now. 

```{r}
Hamilton_data <- Hamilton_data  %>% 
  mutate(Gov_Foreclosure=if_else(AUDPTYID %in% Hamilton_LB$PARCELID, 1,0))
```
...
```{r}
Hamilton_data <- Hamilton_data %>% 
  filter(!PROPTYID=="00960002026600") %>% 
  filter(!PROPTYID=="01750018004700") %>% 
  filter(!PROPTYID=="05000214027100") %>% 
  filter(!PROPTYID=="05000214027900") %>% 
  filter(!PROPTYID=="05500323045100") 
```
...
Hamilton_Distance_Matrix1 <- Hamilton_Distance_Matrix %>% 
  `colnames<-`(Hamilton_data$PROPTYID ) %>% 
  
  #Adds a column containing names so that each row now also has a name
  cbind(name = Hamilton_Geo_Millions$PROPTYID  ) 
#Hamilton_Distance_Matrix1 is the same as Cuyahoga_Distance_Matrix but with the parcel number down the side  and column names on top so we don't get confused. But obviously that is just a label and not actually part of the calculations (aka a bad data practice) which is why we keep Hamilton Distance Matrix as well.
...
Hamilton_Distance_Info1 <-Hamilton_Number_Near %>% 
  select(-name) %>% 
  pivot_longer("06510026000700":"05100031051600", names_to = "PARCELID", values_to = "number_of_high_value_properties_within_half_mile" )
```

```{r}
Hamilton_Distance_Info2 <-Hamilton_Number_Semi_Near %>% 
  select(-name) %>% 
  pivot_longer("06510026000700":"05100031051600", names_to = "PARCELID", values_to = "number_of_high_value_properties_within_mile" )
```
...
```{r}
Hamilton_data <- left_join(Hamilton_data, Hamilton_Distance_Info1, by="PARCELID")
Hamilton_data <- left_join(Hamilton_data, Hamilton_Distance_Info2, by="PARCELID")
```
...
Adding in the amount of delinquency. Remember the properties that didn't pay in 2017 (Certified delinquent in 2018) don't have an assessed value for that year. 

```{r}
Hamilton_data$delinquent_amount <- Hamilton_Delinquent$`delinquent amount`[match(Hamilton_data$AUDPTYID, Hamilton_Delinquent$property_number)]

```

```{r}
summary(Hamilton_data$delinquent_amount)
```
...
stack_overflow <- stack_overflow %>% 
    mutate(Gov_Foreclosure = factor(Gov_Foreclosure, levels = c(1,0))) %>%
    mutate_if(is.character, factor) #%>% filter(delinquent_amount>1023)
   #mutate(Within_mile = factor(Within_half_mile, levels = c(1,0)))


# Create stack_select dataset
stack_select <- stack_overflow %>%
    select("Gov_Foreclosure", delinquent_amount,
      #owed_as_a_percentage_of_lot_value,                                                        
      #"is_school_district_majority_white",
          #"is_school_district_majority_disadvantaged",
...
as.data.frame(Hamilton_data) %>% arrange(desc(delinquent_amount)) %>% top_n(99) %>% summarize(sum(Gov_Foreclosure))
...
```{r}
as.data.frame(Hamilton_data) %>% filter(Gov_Foreclosure==1) %>% summarize(mean(delinquent_amount))
as.data.frame(Hamilton_data) %>% filter(Gov_Foreclosure==1) %>% summarize(median(delinquent_amount))

as.data.frame(Hamilton_data) %>% arrange(desc(delinquent_amount)) %>% top_n(99) %>% summarize(mean(delinquent_amount))
as.data.frame(Hamilton_data) %>% arrange(desc(delinquent_amount)) %>% top_n(99) %>% summarize(median(delinquent_amount))
```

How many were still delinquent three years later?
```{r}
Hamilton_Worst_Offenders <- as.data.frame(Hamilton_data) %>% arrange(desc(delinquent_amount)) %>% top_n(99) %>% filter(AUDPTYID %in% Hamilton_Delinquent$property_number) 

datatable(Hamilton_Worst_Offenders)
```
