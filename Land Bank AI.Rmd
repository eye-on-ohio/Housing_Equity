...
Cuyahoga_Properties_plotted_all <- leaflet(data = Cuyahoga_data_with_map) %>% 
  addTiles() %>% 
  setView(-81.66754692563244,41.43053368069701, zoom = 10) %>% 
  addPolygons(popup =paste("Parcel ID:",Cuyahoga_data_with_map$PARCEL_ID)) %>% 
...
```{r}
Cuyahoga_Delinquents <- Cuyahoga_Delinquents %>%  arrange(desc(GRAND_TOTAL_OWED)) %>% distinct(PROPERTY_NUMBER, .keep_all = TRUE)
```
...
Cuyahoga_Delinquents <- Cuyahoga_Delinquents %>% 
  ...
  filter(PROPERTY_NUMBER!="543-39-072") #Looks like this onw was improperly tagged as residential taxable property and not  parkland
```
...
Note: 68 properties are listed as having a value of 0. This is on the website too, and there doesn't seem to be a clear pattern. Looks like these parcels were Incorporated into others. 
...
So lots of properties went to the government, but that's very few of the tax-delinquent. (And some ostensibly people paid their back taxes!) 
...
They don't allow you to download all the info at once by district, or if they do we couldn't figure it out. But anyway, this takes every school in Cuyahoga. (So we searched for "Cuyahoga," clicked on the button next to each one, then downloaded.) Note: subsequent counties use a file that has this altogether then filters it in R. This is limited to 2021 school numbers. 
...
```{r}
Number_Near <- rio::import("Cuyahoga_Number_Near.csv")

#So you would normally use this fuction, but as it takes a longggg time, we are going to, for review purposes just reload it in here. 
...
```{r}
Number_Semi_Near <- rio::import("Cuyahoga_Number_Near.csv")
#So you would normally use this fuction, but as it takes a longggg time, we are going to, for review purposes just reload it in here. 
...
ggplot(aes(x=GRAND_TOTAL_OWED))+
  geom_histogram()+
  stat_bin(binwidth = 10000, geom="text",aes(label=..count..), position = position_stack(vjust = .9))
...
Next, we are going to split the dataset into training and testing models, evenly diving the sections between the Land Bank and non-land bank properties. Here you can rerun the code several times, being careful not to use two factors that are related to each other.
...
# Create stack_select dataset
stack_select <- stack_overflow %>%
  select(Gov_Foreclosure, GRAND_TOTAL_OWED
         #, owed_as_percentage_of_lot_value
         ,is_school_district_majority_white, number_of_high_value_properties_within_500m 
         ) 
...
```{r}
Cuyahoga_Delinquents_With_Distance_Info_Geo %>% filter(Gov_Foreclosure==1) %>% summarize(mean(GRAND_TOTAL_OWED, na.rm=TRUE))
 
 Cuyahoga_Delinquents_With_Distance_Info_Geo %>% filter(Gov_Foreclosure==1) %>% summarize(median(GRAND_TOTAL_OWED, na.rm=TRUE))
  
Cuyahoga_Delinquents_With_Distance_Info_Geo %>% filter(Gov_Foreclosure==1) %>% summarize(median(owed_as_percentage_of_lot_value, na.rm=TRUE))

Cuyahoga_Delinquents_With_Distance_Info_Geo %>% filter(Gov_Foreclosure==1) %>% summarize(mean(owed_as_percentage_of_lot_value, na.rm=TRUE))

  Cuyahoga_Delinquents_With_Distance_Info_Geo %>% summarize(mean(owed_as_percentage_of_lot_value, na.rm=TRUE))
  
  Cuyahoga_Delinquents_With_Distance_Info_Geo %>% summarize(median(owed_as_percentage_of_lot_value, na.rm=TRUE))
 
   Cuyahoga_Delinquents_With_Distance_Info_Geo %>% count(Gov_Foreclosure)

 Cuyahoga_Delinquents_With_Distance_Info_Geo %>% summarize(mean(GRAND_TOTAL_OWED, na.rm=TRUE))
...
```{r}
Cuyahoga_Delinquents_With_Distance_Info_Geo %>% filter(is_school_district_majority_white==1) %>% summarize(mean(owed_as_percentage_of_lot_value, na.rm=TRUE))
Cuyahoga_Delinquents_With_Distance_Info_Geo %>% filter(is_school_district_majority_white==1) %>% summarize(median(owed_as_percentage_of_lot_value, na.rm=TRUE))
Cuyahoga_Delinquents_With_Distance_Info_Geo %>% filter(is_school_district_majority_white==0) %>% summarize(mean(owed_as_percentage_of_lot_value, na.rm=TRUE))
Cuyahoga_Delinquents_With_Distance_Info_Geo %>% filter(is_school_district_majority_white==0) %>% summarize(median(owed_as_percentage_of_lot_value, na.rm=TRUE))



Cuyahoga_Delinquents_With_Distance_Info_Geo %>% filter(is_school_district_majority_white==1) %>% summarize(mean(Certified_Total_Value, na.rm=TRUE))
Cuyahoga_Delinquents_With_Distance_Info_Geo %>% filter(is_school_district_majority_white==1) %>% summarize(median(Certified_Total_Value, na.rm=TRUE))
Cuyahoga_Delinquents_With_Distance_Info_Geo %>% filter(is_school_district_majority_white==0) %>% summarize(mean(Certified_Total_Value, na.rm=TRUE))
Cuyahoga_Delinquents_With_Distance_Info_Geo %>% filter(is_school_district_majority_white==0) %>% summarize(median(Certified_Total_Value, na.rm=TRUE))

Cuyahoga_Delinquents_With_Distance_Info_Geo %>% filter(is_school_district_majority_white==1) %>% summarize(mean(GRAND_TOTAL_OWED, na.rm=TRUE))
Cuyahoga_Delinquents_With_Distance_Info_Geo %>% filter(is_school_district_majority_white==1) %>% summarize(median(GRAND_TOTAL_OWED, na.rm=TRUE))
Cuyahoga_Delinquents_With_Distance_Info_Geo %>% filter(is_school_district_majority_white==0) %>% summarize(mean(GRAND_TOTAL_OWED, na.rm=TRUE))
Cuyahoga_Delinquents_With_Distance_Info_Geo %>% filter(is_school_district_majority_white==0) %>% summarize(median(GRAND_TOTAL_OWED, na.rm=TRUE))
```
...
```{r}
Cuyahoga_Delinquents_With_Distance_Info_Geo %>% filter(is_school_district_majority_white==1) %>% filter(Gov_Foreclosure==1) %>% summarize(mean(owed_as_percentage_of_lot_value, na.rm=TRUE))
Cuyahoga_Delinquents_With_Distance_Info_Geo %>% filter(is_school_district_majority_white==1) %>% filter(Gov_Foreclosure==1) %>% summarize(median(owed_as_percentage_of_lot_value, na.rm=TRUE))
Cuyahoga_Delinquents_With_Distance_Info_Geo %>% filter(is_school_district_majority_white==1) %>% filter(Gov_Foreclosure==1) %>% summarize(mean(GRAND_TOTAL_OWED, na.rm=TRUE))
Cuyahoga_Delinquents_With_Distance_Info_Geo %>% filter(is_school_district_majority_white==1) %>% filter(Gov_Foreclosure==1) %>% summarize(median(GRAND_TOTAL_OWED, na.rm=TRUE))




Cuyahoga_Delinquents_With_Distance_Info_Geo %>% filter(is_school_district_majority_white==0) %>% filter(Gov_Foreclosure==1) %>% summarize(mean(owed_as_percentage_of_lot_value, na.rm=TRUE))
Cuyahoga_Delinquents_With_Distance_Info_Geo %>% filter(is_school_district_majority_white==0) %>% filter(Gov_Foreclosure==1) %>% summarize(median(owed_as_percentage_of_lot_value, na.rm=TRUE))
Cuyahoga_Delinquents_With_Distance_Info_Geo %>% filter(is_school_district_majority_white==0) %>% filter(Gov_Foreclosure==1) %>% summarize(mean(GRAND_TOTAL_OWED, na.rm=TRUE))
Cuyahoga_Delinquents_With_Distance_Info_Geo %>% filter(is_school_district_majority_white==0) %>% filter(Gov_Foreclosure==1) %>% summarize(median(GRAND_TOTAL_OWED, na.rm=TRUE))
```
...
```{r}
summary(Cuyahoga_Delinquents_With_Distance_Info_Geo$GRAND_TOTAL_OWED, na.rm=TRUE)
```
What's the standard deviation?

```{r}
sd(Cuyahoga_Delinquents_With_Distance_Info_Geo$GRAND_TOTAL_OWED, na.rm=TRUE)
```
Let's look at properies around the mean. This is all properties: 
```{r}
Cuyahoga_Delinquents_With_Distance_Info_Geo %>% 
...
```{r}
Cuyahoga_Delinquents_With_Distance_Info_Geo %>% 
  summarize(mean(is_school_district_majority_white, na.rm=TRUE))
...
```{r}
Cuyahoga_Delinquents_With_Distance_Info_Geo %>% 
  summarize(mean(is_school_district_majority_disadvantaged, na.rm=TRUE))
...
Allen_Properties_plotted<- leaflet(data = Allen_LB_Geo) %>% 
  addTiles() %>% 
  setView( -84.10603998384539, 40.73151049471383, zoom = 11) %>% #Set to Lima Center
  addPolygons(data=Allen_LB_Geo$geometry, options = tileOptions(minZoom = 0, maxZoom = 14, continuousWorld = T), popup = paste("Parcel Number:",Allen_LB_Geo$PARCEL))
...
```{r}
Allen_data %>% filter(`Delinquency_Amount`>0) %>% summarize(median(Delinquency_Amount, na.rm= TRUE))
Allen_data %>% filter(`Delinquency_Amount`>0) %>% summarize(mean(Delinquency_Amount, na.rm= TRUE))

Allen_data %>% filter(`Delinquency_Amount`>0) %>% filter(Gov_Foreclosure==1) %>% summarize(median(Delinquency_Amount, na.rm= TRUE))
Allen_data %>% filter(`Delinquency_Amount`>0) %>% filter(Gov_Foreclosure==1) %>% summarize(mean(Delinquency_Amoun, na.rm= TRUEt))


Allen_data %>% filter(`Delinquency_Amount`>0) %>% filter(Gov_Foreclosure==0) %>% summarize(median(Delinquency_Amount, na.rm= TRUE))
Allen_data %>% filter(`Delinquency_Amount`>0) %>% filter(Gov_Foreclosure==0) %>% summarize(mean(Delinquency_Amount, na.rm= TRUE))

Allen_data %>% filter(`Delinquency_Amount`>0) %>% filter(Gov_Foreclosure==1) %>% summarize(median(Years_Delinquent, na.rm= TRUE))
Allen_data %>% filter(`Delinquency_Amount`>0) %>% filter(Gov_Foreclosure==1) %>% summarize(mean(Years_Delinquent, na.rm= TRUE))


Allen_data %>% filter(`Delinquency_Amount`>0) %>% filter(Gov_Foreclosure==0) %>% summarize(median(Years_Delinquent, na.rm= TRUE))
Allen_data %>% filter(`Delinquency_Amount`>0) %>% filter(Gov_Foreclosure==0) %>% summarize(mean(Years_Delinquent, na.rm= TRUE))
...
Allen_data %>% filter(`Delinquency_Amount`>0) %>% filter(Gov_Foreclosure==1) %>% summarize(median(Delinquency_Amount, na.rm= TRUE))
Allen_data %>% filter(`Delinquency_Amount`>0) %>% filter(Gov_Foreclosure==1) %>% summarize(mean(Delinquency_Amount, na.rm= TRUE))
...
ggplot(Montgomery_LB, aes(x=year(Date_Recorded_Or_Disposed), fill=Status))+
         geom_bar()+
    labs(title="Land Bank Properties in Montgomery County", caption="Source: Montgomery County Land Bank public records request \n Graphic by Eye on Ohio, the Ohio Center for Journalism")+
  xlab("Year")+
...
Montgomery_Properties_plotted <- leaflet(data = Montgomery_LB_Geo) %>% 
  ...
  addPolygons(popup = paste("Parcel Number:",Montgomery_LB_Geo$TAXPINNO))
...
573 of these 34,626 properties have a net delinquency of zero. Two of which go to the Land bank, so they may have been gifted.Land banks can of course accept gifted properties, but that's not what we are looking at in this project. 
```{r}
Montgomery_data <- Montgomery_data %>% 
  filter(NETDELQ>0)
```
...
stack_glm
```
Net delinquency not very practically significant
...
Montgomery_data %>%  top_n(278, owed_as_a_percentage_of_lot_value) %>% summarize(sum(Gov_Foreclosure, na.rm=TRUE))

Montgomery_data %>% top_n(278, NETDELQ) %>% summarize(sum(Gov_Foreclosure, na.rm=TRUE))
...
```{r}
Montgomery_data %>% filter(Gov_Foreclosure==1) %>% summarize(mean(NETDELQ, na.rm=TRUE))
Montgomery_data %>% filter(Gov_Foreclosure==1) %>% summarize(median(NETDELQ, na.rm=TRUE))

Montgomery_data %>% top_n(278, NETDELQ) %>% summarize(mean(NETDELQ, na.rm=TRUE))
Montgomery_data %>% top_n(278, NETDELQ) %>% summarize(median(NETDELQ, na.rm=TRUE))
```
