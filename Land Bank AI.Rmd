...
Montgomery_data %>% arrange(desc(owed_as_a_percentage_of_lot_value)) %>% top_n(278) %>% summarize(sum(Gov_Foreclosure))

Montgomery_data %>% arrange(desc(NETDELQ)) %>% top_n(278) %>% summarize(sum(Gov_Foreclosure))
...
Montgomery_data %>% arrange(desc(NETDELQ)) %>% top_n(278) %>% summarize(mean(NETDELQ))
Montgomery_data %>% arrange(desc(NETDELQ)) %>% top_n(278) %>% summarize(median(NETDELQ))
```

How many were still delinquent three years later?
```{r}
Montgomery_Worst_Offenders <- Montgomery_data %>% arrange(desc(NETDELQ)) %>% top_n(278) %>% filter(`Parcel Number` %in% Montgomery_Delinquents_to_2021$`PARCELID      `) 

datatable(Montgomery_Worst_Offenders)
```
